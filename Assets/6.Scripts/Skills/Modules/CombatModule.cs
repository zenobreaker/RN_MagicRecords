...
public class Module_RapidFire : SkillModule
{
    ...
    {
        DamageData finalDamageData = GetEffectiveDamageData(phaseSkill);
        bool isCrit = skill != null && skill.Blackboard.GetValue<bool>("isCrit", false);
        ...
        try
        {
            for (int i = 0; i < totalShots; i++)
            {
                ...
                if (i < totalShots - 1)
                {
                    // 아직 쏠 총알이 남았다면 연사 간격만큼만 대기
                    await UniTask.Delay(TimeSpan.FromSeconds(fireInterval), cancellationToken: token);
                }
                else
                {
                    // 마지막 총알이라면, 애니메이션이 끝까지 재생될 수 있도록 충분히 기다려줍니다.
                    // (actionData.ActionDuration 같은 변수가 없다면 대략적으로 배속된 시간만큼 대기)
